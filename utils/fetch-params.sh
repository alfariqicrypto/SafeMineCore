...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/SafeminemoreParams"
else
    PARAMS_DIR="$HOME/.safeminemore-params"
fi
...
DOWNLOAD_URL="https://github.com/safeminemore/safeminemore/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with SM2R_DISABLE_SOMETHING=1
SM2R_DISABLE_WGET="${SM2R_DISABLE_WGET:-}"
#SM2R_DISABLE_IPFS="${SM2R_DISABLE_IPFS:-}"
SM2R_DISABLE_CURL="${SM2R_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$SM2R_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$SM2R_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$SM2R_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Safeminemore parameters!
...
main() {
    ...
    cat <<EOF
SM2R - fetch-params.sh

This script will fetch the Safeminemore SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Safeminemore SNARK parameters. Note that it is
